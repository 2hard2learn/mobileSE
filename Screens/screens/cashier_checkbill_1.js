...
export const Cashier_Checkbill_1 = ({navigation,route}) => {
  ...
  const renderCard = (item) => {
    return (
        <TouchableOpacity style={{borderWidth:0,width:330,height:75,backgroundColor:'#FFB156',borderRadius:5,justifyContent:'center'}}
          onPress={()=>{
            navigation.navigate({name:'Cashier_Checkbill_2',params:item})
          }}
        >
  ...
  return (
    <View style={{flex:1,paddingTop: Constants.statusBarHeight}}>
      ...
      <View style={{height:80,backgroundColor:'#05C3FF',alignItems:'center',flexDirection:'row'}}>
        ...
        <Text style={{borderWidth:0,fontSize:25,color:'white',marginLeft:20,fontFamily:'Sound-Rounded'}}>เช็คบิล</Text>
      </View>

      <View style={{height:50,borderWidth:0,justifyContent:'center'}}>
        <Text style={{fontFamily:'Sound-Rounded',fontSize:25,color:'#05C3FF',marginLeft:10}}>งานที่ยังไม่ได้เช็คบิล</Text>
      </View>
